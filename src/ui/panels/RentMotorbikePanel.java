...
public class RentMotorbikePanel extends JPanel {
    ...
    private void initUI() {
        ...
        lDays.setFont(new Font("Segoe UI", Font.BOLD, 13)); lDays.setForeground(C_PRIMARY);
        lblDays = new JLabel("—");
        lblDays.setFont(new Font("Segoe UI", Font.PLAIN, 13));

        JLabel lTotal = new JLabel("Tổng tiền:");
        lTotal.setFont(new Font("Segoe UI", Font.BOLD, 13)); lTotal.setForeground(C_PRIMARY);
        lblTotal = new JLabel("—");
        lblTotal.setFont(new Font("Segoe UI", Font.BOLD, 18));
        lblTotal.setForeground(C_SUCCESS);

        summaryBox.add(lDays);  summaryBox.add(lblDays);
        summaryBox.add(lTotal); summaryBox.add(lblTotal);
        ...
        wc.weightx = 1.0; wc.weighty = 0;
        wc.anchor  = GridBagConstraints.NORTH;
        wc.gridy   = 0;
        wrapper.add(card, wc);
        // Filler để đẩy card lên trên
        wc.gridy  = 1; wc.weighty = 1.0; wc.fill = GridBagConstraints.BOTH;
        ...
        txtRentDate.addFocusListener(new java.awt.event.FocusAdapter() {
            @Override public void focusLost(java.awt.event.FocusEvent e) { calculateTotal(); }
        });
    ...
    public void loadMotorbikes() {
        ...
        for (Motorbike m : available) cbMotorbike.addItem(m);
        updatePriceHint();
        calculateTotal();
    }
    ...
    private void updatePriceHint() {
        ...
        if (m == null) { lblPricePerDay.setText(" "); return; }
    ...
    private void calculateTotal() {
        Motorbike m = (Motorbike) cbMotorbike.getSelectedItem();
        if (m == null) { lblDays.setText("—"); lblTotal.setText("—"); return; }
        try {
    ...
    private void handleBooking() {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(this, "Ngày không hợp lệ!\nDùng định dạng: yyyy-MM-dd (VD: 2026-05-20)", "Lỗi", JOptionPane.ERROR_MESSAGE);
            return;
        }
